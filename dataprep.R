...
Data <- Data %>% setNames(gsub("mean\\.\\.", "Mean", names(.))) %>% setNames(gsub("std\\.\\.", "Std", names(.))) %>% setNames(gsub("std\\.\\.", "Std", names(.)))
Data <- Data %>%select(-starts_with("angle"))
#Summary
summary1 <- Data %>% group_by(Subject.ID, Activity.Name)
summary1 <- summary1 %>% summarize_each(funs(mean))
write.table(summary1,file="summary-data.txt",row.name=FALSE)
print(summary1)